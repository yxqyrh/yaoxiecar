...
-(void)setViewsWithdelegate:(id<SGFocusImageFrameDelegate>)delegate focusImageItems:(SGFocusImageItem *)firstItem, ...
{
    NSMutableArray *imageItems = [NSMutableArray array];
    SGFocusImageItem *eachItem;
    va_list argumentList;
...
-(void)clickPageImage:(SGFocusImageItem *)sender{
    NSLog(@"click button tag is %d",sender.tag);
    if (self.delegate && [self.delegate conformsToProtocol:@protocol(SGFocusImageFrameDelegate)]) {
        [self.delegate foucusImageFrame:self  didSelectItem:sender];
    }
}
